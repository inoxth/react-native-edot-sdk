...
| File                                    | Role                                                                                                                                                                                                                                                                                                                                 |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `EdotReactNative.swift`                 | Main RN module. `@objc` bridge methods (`initialize`, `startSpan`, `endSpan`, `setSpanAttribute*`, `recordSpanException`, `recordMetric`, `emitLog`, `reportJsException`, `setTrackingConsent`, `getCurrentSessionId`).                                           |
...
| `EdotReactNativeAgent.swift`            | Pre-init helper for AppDelegate. Accepts `secretToken`, `apiKey`, `sessionSamplingRate`, `exportProtocol`, `persistencePreset` (mutex + range-validated). Double-sets `deployment.environment` keys in `OTEL_RESOURCE_ATTRIBUTES`, calls `ElasticApmAgent.start(...)`. |
