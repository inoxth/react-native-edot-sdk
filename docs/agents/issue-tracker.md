...
- **Project**: React Native EDOT SDK
- **Team**: DELI Dev Tasks (key `DEV`)

Resolve the project and team **by name** at runtime (`list_projects` / `list_teams` with a `query`) — the MCP is already authenticated to the right workspace, so no IDs, workspace slug, or URLs are hardcoded here. Always create issues under the **DELI Dev Tasks** team, attached to the **React Native EDOT SDK** project.

## Conventions

- **Create an issue**: `save_issue` with `title`, `description` (markdown), `team: "DELI Dev Tasks"`, `project: "React Native EDOT SDK"`. `save_issue` both creates and updates — omit `id` to create.
- **Read an issue**: `get_issue` by id or `DEV-<n>` identifier.
- **List issues**: `list_issues` filtered by `project: "React Native EDOT SDK"` (and `team`/`label`/`state` as needed).
...
Create a Linear issue via `save_issue` under team **DELI Dev Tasks**, project **React Native EDOT SDK**.
