...
**Android** (`packages/react-native/android/`): Kotlin module bridging JS to `apm-agent-android` v1.5.0. Supports Old Arch (`ReactContextBaseJavaModule` + `@ReactMethod`) and New Arch (codegen-generated `NativeEdotReactNativeSpec`) via arch-conditional source sets, all delegating to a shared `EdotReactNativeModuleImpl.kt`. Two agent-start paths (`preInitialize` from `MainApplication`, `buildFromJsConfig` from JS init) both register the user/session/global span-attribute interceptor before any user-supplied redactor. `EdotAppMetrics.kt` and `EdotSystemMetrics.kt` fill the `application.launch.time` / `system.cpu.usage` / `system.memory.usage` gaps that `apm-agent-android` v1.5.0 doesn't auto-emit. See [`packages/react-native/android/AGENTS.md`](./packages/react-native/android/AGENTS.md) for the full Android module layout, load-bearing rules, and anti-patterns.
