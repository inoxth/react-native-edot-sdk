...
## Purpose
Define the native module layer that bridges JavaScript to the EDOT iOS and Android SDKs, supporting both old and new React Native architectures.
...
- **WHEN** a React Native app adds `@inoxth/react-native-edot-sdk` and runs `pod install`
...
- **WHEN** a React Native app adds `@inoxth/react-native-edot-sdk` and syncs Gradle
- **THEN** the `EdotReactNative` native module is available via `NativeModules` or TurboModuleRegistry
...
- **THEN** the native modules conform to the generated spec interface, including the new `instrumentationName` argument
...
- **WHEN** a caller invokes `EdotNativeModule.startSpan(name, attrs, parentSpanId, "@inoxth/react-native-edot-navigation")`
- **THEN** the span SHALL be started on a tracer obtained with `instrumentationName = "@inoxth/react-native-edot-navigation"`
- **AND** the span SHALL appear on the wire with `instrumentation.scope.name = "@inoxth/react-native-edot-navigation"`
