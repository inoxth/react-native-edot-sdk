...
### Requirement: Legacy `view.*` attributes on network spans
**Reason:** Renamed to `screen.name` / `screen.id` for cross-platform parity with apm-agent-android (via opentelemetry-android).
